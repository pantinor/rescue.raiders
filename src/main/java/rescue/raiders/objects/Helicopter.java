...
public class Helicopter extends Movable implements InputProcessor {
	...
	public Helicopter(String name, TextureAtlas atlas, float frameRate, float scale, boolean flip) {
		
		super(name, atlas, frameRate, scale, flip);
		
		Array<AtlasRegion> east = atlas.findRegions(name);
		for (TextureRegion tr : east) {
			tr.flip(true, false);
		}
		
		this.flipped = new Animation(frameRate, east);
		
		this.turningRight = atlas.findRegions("turning");
		this.turningLeft = atlas.findRegions("turning");
