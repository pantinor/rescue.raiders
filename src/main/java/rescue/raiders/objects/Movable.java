...
public class Movable extends Actor {
	...
	public Movable(String name, TextureAtlas atlas, float scale, boolean flip) {
		super();
		setName(name);
		this.scale = scale;
		this.flip = flip;
		this.tr = atlas.findRegion(name);
		tr.flip(flip, false);
		this.hitbox = new Rectangle(0, 0, tr.getRegionWidth() * scale, tr.getRegionHeight() * scale);
		this.setUserObject(createMiniIcon(Color.WHITE, 4,4));
	}

	public Movable(String name, TextureAtlas atlas, float frameRate, float scale, boolean flip) {
		super();
		setName(name);
		this.scale = scale;
		this.flip = flip;
	...
	public TextureRegion createMiniIcon(Color c, int w, int h) {
